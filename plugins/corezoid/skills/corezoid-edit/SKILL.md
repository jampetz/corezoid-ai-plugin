...
- Leave new nodes at placeholder coordinates `x: 0, y: 0` — `push-process` auto-places them near their graph neighbours (preserve mode: existing nodes keep their positions). Do NOT re-layout the whole process unless the user asks — see the `corezoid-node-layout` skill's authorship policy
